...
-- Skipped if the link already has an advertiser explicitly assigned, or
-- if there's no ad_placement to pull from. Safe to re-run.
UPDATE short_links sl
   SET advertiser_id = ap.advertiser_id
  FROM ad_placements ap
 WHERE sl.ad_placement_id IS NOT NULL
   AND sl.advertiser_id   IS NULL
   AND ap.id              =  sl.ad_placement_id
   AND ap.advertiser_id   IS NOT NULL;
