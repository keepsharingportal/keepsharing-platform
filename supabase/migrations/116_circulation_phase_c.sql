...
-- ── Publications table (master record for each magazine) ────────────────
-- Migration 114 added website + issuu_url assuming this existed. Make sure
-- it does. Per-publication metadata (print run, holdback, brand color)
-- lives here and the Publications admin page edits these rows.
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_publications_short_name
  ON publications (short_name);

-- Seed RRP + Boom if not already there.
INSERT INTO publications (short_name, name, abbrev, color_hex, print_total, holdback, sort_order)
SELECT * FROM (VALUES
