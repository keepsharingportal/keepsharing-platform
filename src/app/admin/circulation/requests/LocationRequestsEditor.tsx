    ...
    <div className="space-y-4">
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 p-8 text-center bg-white">
          <p className="text-sm text-gray-500">No {activeStatus} requests.</p>
        </div>
      ) : (
            ...
            <li key={r.id} className="rounded-xl border border-gray-200 bg-white p-3">
              <div className="flex flex-wrap items-start gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-gray-900">{r.business_name}</p>
                  ...
                  {(r.contact_name || r.contact_phone || r.contact_email) && (
                    <div className="mt-1.5 flex items-center gap-3 flex-wrap text-[11px] text-gray-700">
                  ...
                  {r.notes && <p className="text-[11px] text-gray-700 mt-1 italic">📝 {r.notes}</p>}
                </div>
                ...
                {r.status === 'pending' && (
                  <div className="shrink-0 flex flex-col gap-1.5">
                    ...
                    <button
                      onClick={() => del(r.id)}
                      disabled={busy !== null}
                      className="inline-flex items-center gap-1 px-2 py-1 text-[11px] text-gray-400 hover:text-red-600 disabled:opacity-50"
                    >
