  ...
  if (loading) {
    return (
      <div className="flex-1 min-h-0 overflow-y-auto p-6 pb-16">
        <p className="text-sm text-portal-muted max-w-[800px] mx-auto">Loading…</p>
      </div>
    )
  }
    ...
    <div className="flex-1 min-h-0 overflow-y-auto p-6 pb-16">
      <div className="max-w-[900px] mx-auto space-y-6">
        ...
        <header className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <Link href="/admin/ads" className="text-xs text-portal-sub hover:text-portal-text inline-flex items-center gap-1 mb-2">
              <ArrowLeft size={12} /> Back to all bookings
            </Link>
            <div className="flex items-center gap-2 mb-1">
              <Pencil size={18} className="text-portal-blue shrink-0" />
              <span className="text-[11px] font-bold uppercase tracking-wider text-portal-sub">Edit placement</span>
            </div>
            ...
            <h1 className="text-2xl md:text-3xl font-black text-portal-text tracking-tight leading-tight">
            ...
            {(() => {
              ...
              if (ad.ad_headline && !ad.advertiser_account_id) {
                return (
                  <p className="text-base text-portal-sub mt-1 italic">
            ...
            <div className="text-xs text-portal-sub mt-2 space-y-0.5">
              {def?.recommendedImageSize && (
                <p><strong className="text-portal-text">Recommended size:</strong> {def.recommendedImageSize}</p>
              )}
              <p>
                Slug: <code className="text-[11px] text-portal-muted">{ad.placement_type}</code>
                {ad.context_slug && <> · Page: <code className="text-[11px] text-portal-muted">{ad.context_slug}</code></>}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link
              href="/admin/ads"
              className="text-sm font-semibold text-portal-text bg-white border border-portal-border rounded-lg px-4 py-2 hover:bg-portal-bg"
            >
              Cancel
            </Link>
            <button
        ...
        {def && (
          <section className="bg-white border border-portal-border rounded-2xl overflow-hidden">
            ...
            <div className="p-5 grid md:grid-cols-2 gap-5">
              <div className="space-y-2 text-sm text-portal-text leading-relaxed">
                <p>{def.whereItAppears}</p>
                <p className="text-xs text-portal-sub italic">{def.description}</p>
              </div>
        ...
        <Section title="Creative">
          {def?.allowsImageMode !== false && (
            ...
            >
              ...
              {def?.recommendedImageSize && (
                <p className="text-[11px] text-portal-sub mt-1.5">
          ...
          <Row
            label="CTA link"
            hintNode={<span className="text-portal-red font-semibold">(generate tracked link below for clicks to count in reports)</span>}
          >
          ...
          >
            ...
            <Row label="Accent color" hint="Hex color (#ef6442). Banner background tint + button color. Falls back to the column's brand color if blank.">
              <div className="flex items-center gap-2">
                ...
                {ad.accent_color && (
                  <span
                    className="shrink-0 w-9 h-9 rounded-md border border-portal-border"
        ...
        <details className="bg-white border border-portal-border rounded-2xl overflow-hidden">
          ...
          <div className="p-5">
            ...
            <Row label="Placement type">
              ...
              {def && (
                <p className="text-[11px] text-portal-sub mt-1.5 leading-relaxed">{def.description}</p>
              )}
            </Row>
          </div>
        </details>
        ...
        {def && def.category === 'in-article' && (
          ...
          >
            <Row label="Size">
              ...
              <p className="text-[11px] text-portal-sub mt-1.5 leading-relaxed">
...
const inp = 'w-full text-sm px-3 py-2 border border-portal-border rounded-lg bg-white outline-none focus:border-portal-blue transition-colors'
  ...
  return (
    <div className="grid sm:grid-cols-[180px_1fr] gap-2 sm:gap-4 items-start">
      <div className="pt-2">
        ...
        {hint && <p className="text-[11px] text-portal-sub mt-0.5">{hint}</p>}
      ...
      <div className="p-5 space-y-4">
        <Row label="Advertiser" hint="Pick from your customer list. Need to create one first? Open Advertisers.">
          <select
            ...
            className="w-full text-sm px-3 py-2 border border-portal-border rounded-lg bg-white outline-none focus:border-portal-blue"
          >
        ...
        {selected && (
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="bg-portal-bg rounded-lg p-3 border border-gray-100">
              ...
              <p className="text-sm font-bold text-portal-text">{selected.business_name}</p>
              {selected.slug && (
                <Link
            ...
            <div className="bg-portal-bg rounded-lg p-3 border border-gray-100">
              ...
              {selected.contact_name && <p className="text-sm font-semibold text-portal-text">{selected.contact_name}</p>}
              ...
              {selected.contact_phone && (
                <a href={`tel:${selected.contact_phone}`} className="block text-xs text-portal-text">
                  {selected.contact_phone}
                </a>
              )}
              {!selected.contact_name && !selected.contact_email && !selected.contact_phone && (
                <p className="text-xs text-portal-muted italic">No contact info on file.</p>
              )}
            </div>
          </div>
        )}
          ...
          {ads.slice(0, 10).map(other => (
            <li key={other.id} className={`px-4 py-2 flex items-center gap-3 hover:bg-portal-bg ${isExpired ? 'opacity-70' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className={`text-xs font-bold truncate ${isExpired ? 'text-portal-sub' : 'text-portal-text'}`}>
                    {findPlacementType(other.placement_type)?.label ?? other.placement_type}
                  </p>
                  {other.context_slug && (
                    <code className="text-[10px] text-portal-muted font-mono truncate">{other.context_slug}</code>
                  )}
                </div>
                {other.ad_headline && (
                  <p className="text-[11px] text-portal-sub truncate mt-0.5">{other.ad_headline}</p>
                )}
              </div>
              <div className="shrink-0 text-right space-y-0.5">
                {isExpired && other.archived_at ? (
                  <p className="text-[10px] text-portal-sub">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-center">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={copy}
                  className="inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-portal-border bg-white text-xs font-bold text-portal-text hover:bg-portal-bg"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
              <div className="bg-portal-bg rounded-lg p-2.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-portal-sub">Clicks</p>
                <p className="text-base font-black text-portal-text tabular-nums">{link.click_count.toLocaleString()}</p>
              </div>
              <div className="bg-portal-bg rounded-lg p-2.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-portal-sub">Destination</p>
                <p className="text-[11px] text-portal-text truncate font-mono">{link.destination}</p>
              </div>
              <div className="bg-portal-bg rounded-lg p-2.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-portal-sub">Created</p>
                <p className="text-[11px] text-portal-text">{new Date(link.created_at).toLocaleDateString()}</p>
              </div>
            </div>
...
function ImageOnlyPreview({ ad, category, placementType }: {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl shadow-sm border border-portal-border ${maxW}`} style={{ aspectRatio: aspect }}>
      ...
      />
      {ad.ad_link && (
        <span className="absolute top-2 right-2 inline-flex items-center gap-1 text-[9px] font-bold bg-white/90 text-portal-text px-1.5 py-0.5 rounded">
    ...
    <div className="rounded-xl border border-portal-border bg-white p-4 shadow-sm max-w-xl">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {ad.ad_description && (
            <p className="text-sm text-portal-sub mt-1 line-clamp-2 leading-snug">{ad.ad_description}</p>
          )}
  ...
  return (
    <div className="relative w-56 aspect-square rounded-2xl overflow-hidden shadow-sm bg-gray-200">
      {ad.ad_image_url ? (
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[11px] text-portal-sub">(image)</div>
      )}
      ...
      <div className="absolute bottom-3 left-3 right-3 text-white">
        ...
        {ad.ad_cta_label && (
          <span className="inline-flex items-center gap-1 mt-2 text-[10px] font-bold bg-white text-portal-text px-2.5 py-1 rounded-full">
            {ad.ad_cta_label}
          </span>
        )}
      </div>
    ...
    <div className="rounded-xl border-2 border-dashed border-portal-blue/30 bg-portal-blue-lt p-4 max-w-xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <PreviewHeadline text={ad.ad_headline} size="md" />
          {ad.ad_description && (
            <p className="text-xs text-portal-sub mt-0.5 line-clamp-1">{ad.ad_description}</p>
          )}
        </div>
        {ad.ad_cta_label && (
  ...
  return (
    <div className="relative aspect-[3/1] w-full max-w-2xl rounded-2xl overflow-hidden bg-gray-200 shadow-sm">
      {ad.ad_image_url ? (
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[12px] text-portal-sub">(hero image)</div>
      )}
      ...
      <div className="absolute inset-0 flex flex-col justify-center p-6 text-white">
        ...
        {ad.ad_cta_label && (
          <span className="inline-flex items-center gap-1 mt-3 px-4 py-2 bg-white text-portal-text rounded-full text-sm font-bold w-fit">
...
function NewsletterPreview({ ad }: { ad: AdRow }) {
  return (
    <div className="max-w-md mx-auto rounded-lg border border-portal-border bg-white p-5 shadow-sm">
      <p className="text-[10px] font-bold uppercase tracking-widest text-portal-sub mb-2">
        {ad.ad_eyebrow || 'Sponsored'}
      </p>
      {ad.ad_image_url && (
        // eslint-disable-next-line @next/next/no-img-element
...
function FooterPreview({ ad }: { ad: AdRow }) {
  return (
    <div className="rounded-lg border border-portal-border bg-white p-4 max-w-sm">
      <p className="text-[10px] font-bold uppercase tracking-widest text-portal-muted mb-2">Footer partner</p>
      <div className="flex items-center gap-3">
        {ad.ad_image_url ? (
          // eslint-disable-next-line @next/next/no-img-element
        ...
        <span className="text-xs text-portal-text font-semibold">{ad.ad_headline || '(name)'}</span>
      </div>
    </div>
  )
}
...
function PreviewEyebrow({ text }: { text: string | null }) {
  if (!text) return null
  return (
    <p className="text-[10px] font-bold uppercase tracking-widest text-portal-sub mb-0.5">{text}</p>
  )
}
