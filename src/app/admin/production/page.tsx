...
export default async function ProductionHubPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-8">
      <div>
        <h1 className="text-xl font-semibold text-portal-text">Production</h1>
        <p className="text-sm text-portal-sub mt-0.5">Plan, assemble, and ship each market&apos;s monthly issue.</p>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {recentIssues.map(([month, count]) => (
              ...
              >
                ...
                <p className="text-[11px] text-portal-sub">{count === 1 ? 'article' : 'articles'}</p>
              </Link>
            ))}
          </div>
        )}
