            ...
            >
              <div className="flex gap-3 p-3.5">
                <input
                  ...
                  className="w-4 h-4 mt-1 rounded border-portal-border-2 text-portal-blue cursor-pointer shrink-0"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1 flex-wrap">
                    <h3 className="text-sm font-bold text-portal-text leading-tight">{student}</h3>
                    {school && (
                  ...
                  <p className="text-[11px] text-portal-sub mb-1">
                    {grade && <>{grade} · </>}
                    {region && <>{region} · </>}
                    Submitted {fmtDate(r.created_at)}
                  </p>
                  <p className="text-xs text-portal-sub leading-snug line-clamp-2">{whySpecial}</p>
                </div>
              </div>
              ...
              {isExpanded && (
                <div className="px-3.5 pb-3 space-y-2 border-t border-portal-border pt-2.5">
                  ...
                  {achievement && (
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-portal-sub mb-0.5">A notable moment</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-portal-border">
                    <div>
                      ...
                      {relationship && <p className="text-[11px] text-portal-sub">{relationship}</p>}
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-portal-sub">Print original</p>
                      {r.print_image_url ? (
                        <Link
                        ...
                        <p className="text-[11px] text-portal-muted">No upload</p>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  {status === 'new' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={busy}
                        className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold text-portal-red hover:bg-portal-red-lt rounded transition-colors"
                      >
                        Reject
                      </button>
                    </>
