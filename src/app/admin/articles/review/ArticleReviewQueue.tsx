      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="text-sm font-semibold text-portal-text truncate">{article.title}</span>
          ...
          {article.subtitle && (
            <p className="text-xs text-portal-sub truncate mb-1">{article.subtitle}</p>
          )}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link
            ...
            className="p-1.5 rounded text-portal-muted hover:text-portal-blue hover:bg-portal-blue-lt transition-colors"
            title="Preview"
          >
            <ExternalLink size={14} />
          </Link>
          {expanded ? <ChevronUp size={15} className="text-portal-muted" /> : <ChevronDown size={15} className="text-portal-muted" />}
        </div>
      </div>
        ...
        <div className="border-t border-portal-border bg-portal-bg px-4 py-4">
          <div className="flex items-center gap-2 mb-3">
            <Link
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white border border-portal-border-2 text-portal-text hover:bg-portal-bg transition-colors"
            >
              Edit Article
            </Link>
          </div>
          ...
          {article.editorial_review_status !== 'approved' && (
            <>
              <div className="mb-3">
                <label className="text-xs font-semibold text-portal-sub mb-1 block">Editorial notes (optional)</label>
                <textarea
                  value={notes}
                  onChange={e => setNotes(e.target.value)}
                  className="w-full text-xs text-portal-text bg-white border border-portal-border rounded-lg px-3 py-2 outline-none focus:border-portal-blue resize-y min-h-[56px]"
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={acting}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-portal-border-2 text-portal-sub hover:bg-portal-row-hover disabled:opacity-50 transition-colors"
                >
                  <X size={13} /> Reject
                </button>
              </div>
    ...
    <div>
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <input
          ...
          className="ml-auto px-3 py-1.5 text-sm border border-portal-border rounded-lg outline-none focus:border-portal-blue bg-white"
        />
      </div>
