...
export default async function PrintLayoutPage({ searchParams }: Props) {
  ...
  const [placementsRes, advertisersRes, prevCountRes] = await Promise.all([
    supabase
      ...
      .select('id, advertiser_account_id, issue_month, design, directory, size, layout, price, social_budget, layout_notes, specific_months, expires_month, notes, advertiser:advertiser_account_id (business_name)')
