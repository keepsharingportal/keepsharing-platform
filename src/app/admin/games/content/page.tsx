...
export default async function GamesContentPage({
  ...
  return (
    <main className="p-6 max-w-[1200px] mx-auto space-y-6 pb-16">
      ...
      <div>
        <Link href="/admin/games" className="inline-flex items-center gap-1 text-xs font-semibold text-gray-400 hover:text-gray-700 mb-2">
      ...
      {!tableMissing && (
        <>
          ...
          <form className="bg-white border border-gray-200 rounded-2xl px-5 py-4 flex flex-wrap items-end gap-3">
            <div>
              <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1">Game</label>
            ...
            <span className="ml-auto text-xs text-gray-400">{active} active · {rows.length} total in this view</span>
          </form>
          ...
          {game === 'word-search' ? (
            ...
            <section className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-gray-700">Add a new {game} entry — {DIFFICULTY_LABELS[diff as keyof typeof DIFFICULTY_LABELS] ?? diff}</h2>
              </div>
              ...
              {game === 'scramble' && (
                <form action={createScramble} className="p-5 grid md:grid-cols-12 gap-3 items-end">
                  ...
                  <div className="md:col-span-5">
                    <label className="block text-[11px] font-semibold text-gray-500 mb-1">Scrambled letters *</label>
              ...
              {game === 'emoji' && (
                <form action={createEmoji} className="p-5 grid md:grid-cols-12 gap-3 items-end">
                  <input type="hidden" name="difficulty" value={diff} />
                  <div className="md:col-span-5">
                    <label className="block text-[11px] font-semibold text-gray-500 mb-1">Emoji clue *</label>
              ...
              {game === 'trivia' && (
                <form action={createTrivia} className="p-5 space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {[1, 2, 3, 4].map(i => (
                      <div key={i}>
                        <label className="block text-[11px] font-semibold text-gray-500 mb-1">Option {i}{i <= 2 && ' *'}</label>
                        <input name={`option_${i}`} required={i <= 2} placeholder={i === 1 ? '6-8 months' : i === 2 ? '9-15 months' : 'optional'} className={inputCls} />
                      </div>
                    ))}
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-500 mb-1">Correct answer * (must match one of the options exactly)</label>
              ...
              {game === 'memory' && (
                <form action={createMemory} className="p-5 grid md:grid-cols-12 gap-3 items-end">
                  <input type="hidden" name="difficulty" value={diff} />
                  <div className="md:col-span-8">
                    <label className="block text-[11px] font-semibold text-gray-500 mb-1">Icons (space-separated emojis) *</label>
                    <input name="icons" required placeholder="🍼 🧸 🧷 🚗 📚 🎨" className={`${inputCls} text-base`} />
                    <p className="text-[10px] text-gray-400 mt-1">Each icon becomes one pair. Minimum 4 icons. The site mirrors them, so 6 icons → 12-card grid.</p>
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-[11px] font-semibold text-gray-500 mb-1">Pairs to show</label>
              ...
              {game === 'family-connect' && (
                <form action={createFamilyConnect} className="p-5 space-y-4">
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">
                    ...
                    <strong className="text-gray-700">yellow</strong> = easiest, then{' '}
                    <strong className="text-gray-700">green</strong>,{' '}
                    <strong className="text-gray-700">blue</strong>, and{' '}
                    <strong className="text-gray-700">purple</strong> = trickiest (red-herring or wordplay).
                  </p>
                      ...
                      <div key={tone} className="rounded-xl border border-gray-200 overflow-hidden">
                        ...
                        <div className="p-3 grid md:grid-cols-12 gap-2">
                          <div className="md:col-span-4">
                            <label className="block text-[10px] font-semibold text-gray-500 mb-1">Group label *</label>
                            <input name={`label_${tone}`} required placeholder="e.g. Diaper bag essentials" className={inputCls} />
                          </div>
                          {[1, 2, 3, 4].map(i => (
                            <div key={i} className="md:col-span-2">
                              <label className="block text-[10px] font-semibold text-gray-500 mb-1">Word {i} *</label>
          ...
          <section className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 bg-gray-50">
              <h2 className="text-sm font-bold text-gray-700">{game} · {DIFFICULTY_LABELS[diff as keyof typeof DIFFICULTY_LABELS] ?? diff} pool</h2>
            </div>
            {rows.length === 0 ? (
              <p className="p-8 text-center text-sm text-gray-400">No entries yet — add one above.</p>
            ) : (
              <ul className="divide-y divide-gray-100">
                {rows.map(r => (
                  <li key={r.id} className={`p-4 flex items-start gap-3 ${r.weight === 0 ? 'opacity-50' : ''}`}>
                    <div className="flex-1 min-w-0">
                      <pre className="text-xs font-mono whitespace-pre-wrap break-words text-gray-800 mb-1">{JSON.stringify(r.payload, null, 0)}</pre>
                      <p className="text-[10px] text-gray-400">Added {new Date(r.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                    </div>
