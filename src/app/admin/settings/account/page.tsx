        ...
        <section className="bg-white rounded-lg ring-1 ring-gray-200 p-6">
          ...
          <dl className="space-y-3 text-sm">
            <Row icon={<Mail size={14} className="text-portal-muted" />} label="Email">
              <span className="font-semibold text-portal-text">{ctx.email}</span>
            ...
            <Row icon={<Users size={14} className="text-portal-muted" />} label="Markets">
              <span className="font-medium text-portal-text">{marketsLine}</span>
            </Row>
          </dl>
        </section>
...
function Row({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3">
      <dt className="shrink-0 w-24 flex items-center gap-1.5 text-[11px] uppercase tracking-wider font-bold text-portal-sub pt-1">
        {icon} {label}
      </dt>
