  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white print:bg-white">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between print:hidden">
        <Link href={`/admin/advertisers/${id}`} className="text-xs text-gray-500 hover:text-gray-900 inline-flex items-center gap-1">
      ...
      <div className="max-w-4xl mx-auto p-10 md:p-14">
          ...
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {tier && <p className="text-sm text-gray-500 mt-0.5">{tier.replace(/-/g, ' ')}</p>}
              </div>
            </div>
        ...
        <section className="mb-10">
          ...
          {plRows.length === 0 ? (
            <p className="text-sm text-gray-500 italic">No active ad placements this month.</p>
          ) : (
            <div className="rounded-xl ring-1 ring-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="text-left px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Placement</th>
                    <th className="text-right px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Impressions</th>
                    <th className="text-right px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Clicks</th>
                    <th className="text-right px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">CTR</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={p.id}>
                        <td className="px-4 py-3">
                          ...
                          {p.ad_headline && <p className="text-[11px] text-gray-500 mt-0.5 truncate max-w-xs">&ldquo;{p.ad_headline}&rdquo;</p>}
                        </td>
          ...
          <section className="mb-10">
            ...
            <div className="rounded-xl ring-1 ring-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="text-left px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Short Link</th>
                    <th className="text-left px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Label / Campaign</th>
                    <th className="text-right px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Total Scans</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {qrCodes.map(q => (
                    <tr key={q.id}>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-gray-900">/go/{q.shortcode}</p>
                        <p className="text-[11px] text-gray-500 truncate max-w-xs">→ {q.destination}</p>
                      </td>
        ...
        {/* Footer */}
        <div className="mt-12 pt-6 border-t border-gray-200 flex items-center justify-between text-[11px] text-gray-400">
